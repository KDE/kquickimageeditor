...
Item {
    ...
    Binding {
        target: root.tool
        property: "geometry"
        value: Utils.rectNormalized(root.itemRect(selectionItem))
        when: root.dragging
    ...
    DragHandler {
        ...
        onTranslationChanged: if (active) {
            ...
            rect = Utils.rectClipped(rect, root.maxRect())
    ...
    Keys.onPressed: (event) => {
        if (event.key === Qt.Key_Enter || event.key === Qt.Key_Return) {
            root.accepted(root.tool.geometry)
            event.accepted = true
    ...
    Connections {
        ...
        function onUndoStackDepthChanged() {
            let rect = Utils.rectAspectRatioed(root.itemRect(selectionItem), root.tool.aspectRatio)
            rect = Utils.rectClipped(rect, root.maxRect())
            root.setItemRect(selectionItem, rect)
        }
    }
    ...
    Outline {
        ...
        DragHandler {
            ...
            xAxis {
                minimum: 0
                maximum: root.width - root.tool.geometry.width
            }
            yAxis {
                minimum: 0
                maximum: root.height - root.tool.geometry.height
            }
        }
        TapHandler {
    ...
    component ResizeHandle: P.Handle {
        ...
        DragHandler {
            ...
            xAxis {
                ...
                maximum: root.width - handle.width / 2
            }
            yAxis {
                enabled: handle.edges & (Qt.TopEdge | Qt.BottomEdge)
                minimum: -handle.height / 2
                maximum: root.height - handle.height / 2
            }
            onTranslationChanged: (delta) => {
                ...
                if (active && (delta.x !== 0 || delta.y !== 0)) {
                    ...
                    let rect = root.itemRect(selectionItem)
